    ...
    for (let i=1; i < values.length; i++) {                                       // 1.3.2) sort optimization: at each iteration, the last value will be skipped (i)
      for (let j=0; j < values.length - i;) {                                     // 1.3.2.1) 'values' index
        if (!missingProps) {                                                      // 1.2.2.1.1) every object in 'objArr' have all properties in 'objProps'
          ...
          } else if (values[j][k] > values[j+1][k]) {                             // 1.3.2.1.1.1) Case two: first value > second value: swap the two values
            [values[j], values[j+1]] = [values[j+1], values[j]];                  // 1.3.2.1.1.2.1) swap 'values'
            [indices[j], indices[j+1]] = [indices[j+1], indices[j]];              // 1.3.2.1.1.2.2) swap 'indices'
            k = 0; j++;                                                           // 1.3.2.1.1.2.3) restart properties; check next value
          } else {                                                                // 1.3.2.1.1.3) Case three: first value < second value: already sorted, nothing to do
            k = 0; j++;                                                           // 1.3.2.1.1.3.1) restart properties; check next value
          }
        }
        else {                                                                    // 1.2.2.1.2) else, some objects in 'objArr' don't have all properties listed in 'objProps'
          if (values[j][k].toString() === values[j+1][k].toString()) {            // 1.3.2.1.2.1) Case one: values are equal; using string check to test numbers or strings against ''
          ...
          } else if (values[j][k].toString() > values[j+1][k].toString()) {       // 1.3.2.1.2.2) Case two: first value > second value: swap the two values; using string comparison to test numbers or strings against ''
            [values[j], values[j+1]] = [values[j+1], values[j]];                  // 1.3.2.1.2.2.1) swap 'values'
            [indices[j], indices[j+1]] = [indices[j+1], indices[j]];              // 1.3.2.1.2.2.2) swap 'indices'
            k = 0; j++;                                                           // 1.3.2.1.2.2.3) restart properties; check next value
          } else {                                                                // 1.3.2.1.2.3) Case three: first value < second value: already sorted, nothing to do
            k = 0; j++;                                                           // 1.3.2.1.2.3.1) restart properties; check next value
          }
        }
      }
    }
